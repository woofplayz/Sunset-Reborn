...
document.addEventListener("DOMContentLoaded", async () => {
    const a = await fetch(jj).then((response) => response.text()).then((text) => {return JSON.parse(text)});
       a.forEach(z => {
        ...
        aaz.setAttribute('onclick', `openDynamicModal('${z.name}', '${z.description}', '${z.url}')`);
